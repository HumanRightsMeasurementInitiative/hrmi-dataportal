...
const {
  ...
  getIndicators,
  getRights,
} = require('./helpers/generate-files');
...
keys(pages).forEach(code => {
  ...
  $("head meta[property='og:title']").attr(
    'content',
    `${country || right || indicator} - HRMI Rights Tracker`,
  );
  if (country) {
    $("head meta[property='og:image']").attr(
      'content',
      `http://content-store.humanrightsmeasurement.org/assets/uploads/country_${code}.png`,
    );
  } else if (indicator) {
    $("head meta[property='og:image']").attr(
      'content',
      `http://content-store.humanrightsmeasurement.org/assets/uploads/indicator_${code}.png`,
    );
  } else {
    $("head meta[property='og:image']").attr(
      'content',
      `http://content-store.humanrightsmeasurement.org/assets/uploads/right_${code}.png`,
    );
  }
  ...
  fs.writeFileSync(
    path.join(__dirname, `../../app/templates/${code}.html`),
    $.html(),
  );
});

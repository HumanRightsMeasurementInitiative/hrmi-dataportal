...
async function printPDF ({ countries, languages }) {
  ...
  await cluster.task(async ({ page, data: { lang, code, langFile, as } }) => {
    ...
    try {
      await page.pdf({
        ...
        headerTemplate: `${headerFooterStyle} <div style="font-family: 'Source Sans Pro', sans-serif; width: 100%; display: flex; flex-direction: row; justify-content: space-between; align-items: center; margin-left: 35px; margin-right: 35px; margin-top: 12px; margin-bottom: 0;"> <img src=${logo} alt="logo" style="width: 140px"></img> <p style="font-weight: 600">${
          langFile['hrmi.pdf.subtitle']
        } ${langFile[`hrmi.countries.${code}`]}, ${currentYear}</p> </div>`,
    ...
    console.log(
      `${lang}-${code} done, timePagePDF: ${process.hrtime(timePagePDF)[0]}.${
        process.hrtime(timePagePDF)[1]
      } seconds`,
    );
  });
...
(async () => {
  ...
  const dataResponse = await fetch(
    'http://data-store.humanrightsmeasurement.org/data/countries_v3-1.csv',
  );
  ...
  console.log(
    `generate-pdfs done, timePrintPDF: ${process.hrtime(timePrintPDF)[0]}.${
      process.hrtime(timePrintPDF)[1]
    } seconds`,
  );
})();

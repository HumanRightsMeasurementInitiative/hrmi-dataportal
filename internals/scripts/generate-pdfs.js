  ...
  await cluster.task(async ({ page, data: { lang, country, langFile } }) => {
    ...
    try {
      await page.pdf({
        ...
        headerTemplate: `<style> #header {padding: 0 !important} p, span { font-family: SourceSansPro !important; font-size: 10px; color: #262064;}</style><div style="width: 100%; display: flex; flex-direction: row; justify-content: space-between;margin-left: 35px; margin-right: 35px; margin-top: 5px; margin-bottom: 0;"> <img src=${logo} alt="logo" style="width: 120px"></img> <p style="font-weight: 600">${langFile['hrmi.pdf.subtitle']} ${langFile[`hrmi.countries.${country}`]}, ${currentYear}</p> </div>`,
        footerTemplate: `<style> @font-face {font-family: SourceSansPro;src:url(../../fonts/SourceSansPro-Regular.ttf) format("truetype");} p, span {font-family: SourceSansPro !important; font-size: 10px; color: #262064;} #footer { padding: 0 !important; } </style><div style="height: 40px; width: 100%; background-color: #d3d3d3; -webkit-print-color-adjust: exact; display: flex; flex-direction: row; justify-content: space-around;"> <p style="font-weight: 600;">  ©HRMI 2020 </p> <a href="http://rightstracker.org" style="text-decoration: none; color: unset"> <p>rightstracker.org </p> </a> <p style="font-weight: 600;"> Page <span class="pageNumber"></span>/<span class="totalPages"></span> </p></div>`,
        margin: {
          top: "55px",
          bottom: "76px",
        }
